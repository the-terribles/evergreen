 ...
 */
GraphBuilder.prototype.buildDependenciesForLeaf = function(tree, metadata, leafConfig){
  var me = this;
  ...
  return _.map(leafConfig.expression, function(p){
    if (p.type === 'placeholder'){
      var placeholderDependency = metadata.paths[p.value],
          leafSympath = treeUtils.joinPaths(leafConfig.path),
          value = me.resolveNodeAtPath(tree, p.value, placeholderDependency.path, leafSympath, leafConfig.path);
...
GraphBuilder.prototype.resolve = function(leaf, tree, metadata){
  ...
  var dependencies = this.buildDependenciesForLeaf(tree, metadata, leafConfig);

  // Ensure all dependencies are resolved.
  if (!GraphBuilder.allDependenciesAreResolved(dependencies)) return null;
