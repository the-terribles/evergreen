 ...
 * Directive Handler:
 *
 * function({ strategy: directiveName, context: contentAfterDirectiveName, path: leafConfig.path }, callback){
 *    return callback(err, { path: leafConfig.path, value: * });
 * }
 *
...
GraphBuilder.prototype.processTree = function(tree, callback, lastPassOutcome){
  ...
  if (directives.length === 0){
    return this.processTree(tree, callback, GraphBuilder.PassOutcomes.NO_DIRECTIVES_PROCESSED);
  }

  async.parallel(_.map(directives, this.getDirectiveHandler.bind(this)), function(err, results){
    if (err) return callback(err);

    results.forEach(function(result){ GraphBuilder.appendToTree(tree, result.path, result.value); });

    // Next pass.
    me.processTree(tree, callback, GraphBuilder.PassOutcomes.DIRECTIVES_PROCESSED);
  });
};
