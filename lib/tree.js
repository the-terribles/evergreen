...
var evaluate, handleBranch, joinPaths, assertSubpathValid,
    splitPath, findDependencies, getAt, setAt, isIndex, isField, isNamedField;
...
/**
 * Is the subpath a Field?
 * @param subpath {Object} array subpath
 * @returns {Boolean} true if it is a field
 */
module.exports.isField = isField = function(subpath){
  return _.isObject(subpath) && subpath.hasOwnProperty('field') && _.isString(subpath.field);
};

/**
 * Is the subpath an Index?
 * @param subpath {Object} array subpath
 * @returns {Boolean} true if it is an index
 */
module.exports.isIndex = isIndex = function(subpath){
  return _.isObject(subpath) && subpath.hasOwnProperty('index') && _.isInteger(subpath.index) && subpath.index >= 0;
};

/**
 * Is the array subpath a field with the supplied name?
 * @param subpath {Object} array subpath
 * @param name {String} name of the field
 * @returns {Boolean} true if it is a field and named the same as the "name" parameter.
 */
module.exports.isNamedField = isNamedField = function(subpath, name){
  return isField(subpath) && subpath.field === name;
};
...
module.exports.evaluate = evaluate = function(tree, prefix){
  if (!utils.isNonArrayObject(tree)) throw new TreeNotObjectError(tree);
