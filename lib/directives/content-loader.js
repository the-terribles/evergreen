...
ContentLoader.prototype.handle = function(context, tree, metadata, callback){
  this.load(context.expression, function(err, content, noParse){
    if (err) return callback(err);
    if (noParse || !_.isString(content)) return callback(null, context.resolve(content));
    try {
      var value = JSON.parse(content);
      return callback(null, context.resolve(value));
    }
    catch (e){
      return callback(null, context.resolve(content));
    }
  });
};
